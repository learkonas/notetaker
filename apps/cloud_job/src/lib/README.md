...
| `inbox.ts` | HTTP client for the inbox API (`createInboxClient`). Authenticates with Cloudflare Access service-token headers and exposes list/get/move/mark-read/folder operations. Detects redirects to the Access login page and raises a descriptive error. |
